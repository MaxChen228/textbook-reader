...
| `pipeline_tick.py` | 單次 tick：resume in-flight → 買書員 `select_next` 取 QUALIFIED 確定性下載 → 走 actionable。det 直跑，LLM（qc/audit/sol）派 headless `claude -p`（每 tick `--max-llm` 上限）。**crawl 在 daemon 全確定性（只買書員下載既有 QUALIFIED）——填書單/四維查證已不在 daemon，改使用者親打 /restock 互動驅動**。**`--dry-run` 印計劃**。 |
...
| `editions/` + `fields.json` + `booklists.py` | **書目 universe（合格存在四維模型）**：`editions/<slug>.json`（git）= 每本「存在」的書（owned∪已連結∪discovery 候選）的完整記錄〔身份+分類+四維結論〕；`fields.json`（git）= 領域骨架。`booklists.py` 由 editions 派生五態（OWNED/QUALIFIED/PENDING/CANDIDATE/REJECTED）→ catalog/select_next/progress。**沒 editions 的書＝沒連結＝不存在**（落實使用者 model）。買書員 `select_next` 只取 QUALIFIED（四維全過）下載。題本由主書 `identity.has_solution` 衍生 `<slug>_sol`。舊 `booklists/*.json` wishlist 已退役封存於 `booklists/_archive/`。 |
| `resolve.py` + `crawl_resolution.json` + `editions.py` | **/restock 填書單的 harness（使用者互動驅動，非 daemon）**：queue（`--state candidate/pending`）/target/search（候選+advisory，只 search）/inspect/commit（**維②連結二態 found\|not_found**）+ `editions set/show`（維①③④：夠格/版本/解答對齊，LLM 親查、git 貴重成果）。連結態寫 crawl_resolution（純連結 cache）、四維判斷寫 editions（貴重）。`auto` 只自動採用零歧義 exact 主書連結（落 found→PENDING、版本仍待親查）。 |
...
| `references/{booklist-manager,qc,math-sweep}.md` | 流程指引：**填書單/四維查證**（`booklist-manager.md` = /restock 引擎：夠格∧連結∧版本∧解答對齊，多 haiku 多源——**使用者互動 /restock、Claude 親自 fan-out，非 daemon headless**；`crawl.md` 已退役）/ 視覺 QC（headless agent）/ 數學 sweep（**架構師手動參考**——daemon 已直跑 batch，僅難條 `fix` 或寫規則時讀）。 |
