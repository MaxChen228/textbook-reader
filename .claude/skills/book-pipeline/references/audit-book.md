...
```bash
uv run python -m book_pipeline.audit_scout <slug>
```
...
```bash
SLUG=<slug>
uv run python -m book_pipeline.parser $SLUG  # 產出 parsed/chNN.json
uv run python -m book_pipeline.smoke $SLUG   # 啟發式 anomaly 檢測
```
...
| **H6/H7** catalog 圖說/ref 殘留（unresolved refs · fallback id · 空 caption） | **不是你的職責**——圖/表 caption 綁定、無號圖排除、多 image 共用 caption，全由**下游 `catalog_audit` 階段的確定性 `repair_catalog_metadata`** 自動處理（鄰近 ±3 block 抓 caption + 共享 ref 群組 + explicit caption promote） | **不修、不提案**：你在 audit 跑 smoke 看到的 H6/H7 是 **repair 前的暫態**，下一階段 repair 一跑就歸零。僅把現象記入 `_audit.md`，**嚴禁**為 H6/H7 開 engine tooling-gap 提案 |

**iterate 規則**（**只收斂 H1–H5；H6/H7 屬下游、不計入你要過的 critical**）：
1. 第 1 次跑 smoke：若 **H1–H5** 皆綠 → 進 Step 8、結束（縱使 smoke 因 H6/H7 退出碼為 1，你這關也算過）
2. **H1–H5** 有 critical：依表修 yaml → 重跑 §5 schema validator → 重跑 parser → 重跑 smoke
3. 至多 3 輪。第 3 輪仍有 H1–H5 critical 但你已嘗試合理修法 → 接受殘留，把每個未修 critical 列進 `_audit.md` 並加 ⚠ 說明原因（OCR 邊界、跨書罕見 case 等）
...
```bash
SLUG=<slug>
uv run python -m book_pipeline.validate_rules $SLUG
...
```bash
uv run python -m book_pipeline.parser $SLUG
uv run python -m book_pipeline.smoke $SLUG
uv run python -m book_pipeline.normalize_metadata --fix   # 自動修 edition/author 格式 + 驗 subject ∈ schema
```
...
```bash
uv run python -m book_pipeline.extract_cover $SLUG
```
...
- **引擎表達不了你這本書時：停手 + 提案，絕不擅改引擎**。parser / smoke 若無法正確切你這本的
  ...
  **⚠ 語義契約**：開 engine tooling-gap 提案＝**宣告「本書 audit 無法產出 yaml」**（daemon 見『有 engine 提案
  ...
  uv run python -m book_pipeline.proposals propose --domain engine --type tooling-gap \
    ...
    --evidence "<你這本的具體形態，如：inline 習題散落多種 heading、parser 切不開真題與散文編號清單>" \
    --proposal "<你建議引擎怎麼補>"
  ```
