...
- autonomous：泛化提案**只 append 到 `book_pipeline/math_overrides/_proposals.md`**，**絕不**自行改 math_macros.json / math_normalize.py（核心碼、daemon 不 commit、誤改掩蓋真錯）。one-off override 照寫照套。
- 互動 owner：可直接照 §6 升級 Layer 0/1（補測試 + 全 corpus 回歸）。
...
3. **全 corpus 回歸**：升級後 `backfill_math` / `math_validate --all`，**任一書殘餘上升即回退**。
...
升級後 commit（owner 親自，daemon 不 commit）。autonomous 模式：以上只寫成 `_proposals.md` 一節，交人工。
...
- **daemon（autonomous）**：`pipeline_tick.do_math_sweep` 在 corpus 殘餘 ≥ `MATH_SWEEP_THRESHOLD`（預設 50）時派
  `LLM_PROMPTS['math_sweep']`（slug=None，跨書）。agent 寫 override + `_proposals.md`（自跑 apply/validate 驗證即可）；
  **daemon 是 apply 的權威**：收尾對所有 override 確定性 re-apply（idempotent）+ 重烤改動的書上站、重量殘餘、記 sweep
  狀態（防同狀態重派）。故 agent 不必管部署，daemon 重烤集 = 本輪真改動或新寫 override 的書（不依賴 agent 行為）。
