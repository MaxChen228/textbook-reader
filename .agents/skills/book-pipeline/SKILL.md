...
| `pipeline_tick.py` | 單次 tick：resume in-flight → 自書單 SoT 確定性補爬 → 走 actionable。det 直跑，LLM（**crawl 查證**/qc/audit/sol）派 headless `Codex -p`（每 tick `--max-llm` 上限）。crawl 選書/買書員確定性，但**解析（書名→id/hash+版本）是 LLM agent（多源查證 skill）**。**`--dry-run` 印計劃**。 |
...
| `resolve.py` + `crawl_resolution.json` + `editions.py` | **/restock 庫存查證 agent 的 harness**：queue（`--state candidate/pending`）/target/search（候選+advisory，只 search）/inspect/commit（**維②連結二態 found\|not_found**）+ `editions set/show`（維①③④：夠格/版本/解答對齊，LLM 親查、git 貴重成果）。連結態寫 crawl_resolution（純連結 cache）、四維判斷寫 editions（貴重）。`auto` 只自動採用零歧義 exact 主書連結（落 found→PENDING、版本仍待親查）。 |
...
| `references/{booklist-manager,qc,math-sweep}.md` | 流程指引：**庫存查證/四維**（`booklist-manager.md` = /restock 引擎：夠格∧連結∧版本∧解答對齊，多 haiku 多源，headless agent；`crawl.md` 已退役）/ 視覺 QC（headless agent）/ 數學 sweep（**架構師手動參考**——daemon 已直跑 batch，僅難條 `fix` 或寫規則時讀）。 |
