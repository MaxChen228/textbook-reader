...
- **監控**:`/dev` 頁(`dev/index.html`,復用 reader 元件)即時看 daemon/budget/錯誤/書本階段。單一真相源 `book_pipeline/devctl.py`——網頁與 CLI 共用:`devctl status|snapshot|errors|incident|kick|crawl-refill`(`crawl-refill`=手動丟「強制補貨」marker `crawl_refill_request`,daemon 下個 observe cycle 無視水位/冷卻派 crawl 小弟補書單;運轉中不中斷在飛工作、閒置才 kick;外部只丟 marker→不搶寫 crawl_queue.json,UI 化也只換前端)。snapshot 由 `pipeline_tick.log()` 事件驅動刷新(節流 8s)+ `com.textbookreader.devsnapshot` plist 60s 心跳;寫 `dev/status.json`(gitignore)。**出事除錯入口 = `uv run python -m book_pipeline.devctl incident`**。
