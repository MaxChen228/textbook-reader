...
- 確定性階段 daemon 直跑;需判斷的階段(qc/audit/sol_extract)派 headless LLM 跑 `.claude/skills/book-pipeline/references/*.md`。**派工策略單一真相源 = `book_pipeline/llm_policy.py`**(`DispatchSpec` + `DEFAULT_DISPATCH`/`STAGE_DISPATCH` + `resolve_dispatch`/`math_sweep_model`/`effective_chain`,**四層合併 DEFAULT←per-stage←控制檔 chain override←env**):per-stage 可宣告 provider chain/model/reasoning effort/timeout。三 provider failover **codex**(原生 OAuth,最穩)/**codex-pool**(codex CLI 走 ccNexus 池子備援額度)/**claude**(Max 保底),**碼層常態 `codex→codex-pool→claude`**(kimi 已於 2026-06-20 下架:斷線窗 fallback 品質不可靠,寧落 Claude Max);effort 分層(重判斷 high/qc low,僅 codex 家族;crawl 改 claude-only 多源查證、無 effort)。**provider 順序 runtime 切換工具 = `devctl chain`**(控制平面 `book_pipeline/provider_chain.py`):無參數=show(生效鏈+來源 default/override/env)/`chain set <p1> [p2…]`(寫 runtime override `.control/provider_chain.json`,凌駕碼層常態、低於 env,逗號分隔亦可)/`chain reset`(清 override 回常態);每次寫後 SIGUSR1 秒級生效、**免改碼/免 commit/免重部署**(換主力 provider 不必 commit `DEFAULT_DISPATCH`)。override **fail-open**(壞檔/未知 provider 退碼層 DEFAULT——與 gates fail-safe hold 刻意相反:派工絕不因壞檔癱瘓);`KNOWN_PROVIDERS` 單一真相在 provider_chain(llm_policy reexport 破循環)。math sweep 走 ccNexus HTTP batch(執行路徑非 CLI)但**模型同源於此配置層**(`math_sweep_model`)。env(`BOOK_PIPELINE_PROVIDER_CHAIN`/`_CODEX_MODEL`/`_CODEX_EFFORT`/`_CLAUDE_MODEL`/`_LLM_TIMEOUT`/`_MATH_MODEL`)為**最高優先**運維單次逃生口(`_MATH_MODEL` 為 math sweep 專屬覆寫;未設時 `_CODEX_MODEL` 亦連動 math sweep)。
...
- **`/dev` 純觀測(2026-06-23,原 devcontrol 寫回 sidecar 已退役)**:`/dev` 只觀測、無任何寫回 UI;所有控制改走 CLI(架構師面)。原 docker compose 第二服務 `devcontrol`(`dev_control.py`)、nginx `location ^~ /dev/api/`、面板暫停鈕/zlib toggle 全移除。crawl 流量控制改由 per-book×per-stage 閘門(`devctl gates allow/hold <slug|*> crawl`,見下「閘門控制」段);**zlib 帳號 disable 系統已完整移除(2026-06-23,commit df63e6d)**——停 N 帳號降上限力度粗(僅 10 倍數)、不直觀又與閘門打架(兩套流量控制);`crawl_zlib accounts/limits` 純觀測真實 z-lib 餘額(10×N=100/日自然上限),/dev 額度 bar 純顯示。
