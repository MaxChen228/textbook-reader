...
- **`/dev` 純觀測(2026-06-23,原 devcontrol 寫回 sidecar 已退役)**:`/dev` 只觀測、無任何寫回 UI;所有控制改走 CLT(架構師面)。原 docker compose 第二服務 `devcontrol`(`dev_control.py`)、nginx `location ^~ /dev/api/`、面板暫停鈕/zlib toggle 全移除。流量控制(zlib 帳號停用)純 CLI `crawl_zlib disable/enable/accounts`(SoT=`zlib_control_state`,/dev 只讀觀測額度 bar)。
