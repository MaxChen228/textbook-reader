...
涵蓋：targets 攤平與解答本衍生/排序、五態衍生、select_next 確定性選書、progress 統計、
validate（dup/非法 slug/缺欄）、reconcile_owned（inventory↔SoT 對賬）。
...
def test_status_five_states():
    ...
    assert bl.status_of('rudin_sol', have, queued, resolution) == bl.UNRESOLVED
    # owned 優先於 resolution（即使有解析，已 owned 就 owned）
    assert bl.status_of('jackson', have, queued, {'jackson': {'id': '9', 'hash': 'z'}}) == bl.OWNED
    print('✓ status：owned>queued>ready/absent/unresolved 五態 + owned 優先')
