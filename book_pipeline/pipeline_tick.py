...
def _schedule_respawn() -> None:
    """丟一個 detached 小弟，**等本 controller 退出（.tick.lock 釋放）後**才 `launchctl kickstart`
    拉起新碼 → 退出即刻 respawn、零空檔。為何要等死：鎖是 LOCK_EX|LOCK_NB，舊實例還活著時 kickstart
    的新實例會搶不到鎖而「跳過本次」→ 必須等舊的死透。**reload 與 walltime 自然退出都呼叫**
    （2026-06-24：walltime 退出原僅靠 launchd StartInterval ≤15min 兜底 → 每 ~50min 週期有 idle-gap、
    per-book 推進全停 ~15min＝~25% 閒置浪費，dogfood 整夜手動 kick 4+ 次才補上；改 walltime 也排即時
    respawn 消 gap）；**idle/terminate 不呼叫**——idle＝真沒事該收斂停（呼叫會 busy-loop）、terminate＝
    kickstart -k 自帶重拉或純 kill 由 StartInterval 兜底（避免雙重重拉）。若與 launchd StartInterval
    fire 撞期 → NB 鎖天然序列化（一個拿到跑、另一個跳過），不雙跑；排程失敗亦退回 StartInterval backstop。"""
    pid = os.getpid()
    uid = os.getuid()
...
def tick_reactive(no_deploy: bool) -> int:
    ...
    try:
        while time.monotonic() < deadline:
            ...
            wake.wait(LOOP_POLL)
            wake.clear()
        # walltime 自然到期退出（迴圈條件 deadline 到，非 reload/idle/terminate break）→ 排 detached 立即
        # respawn 消 ~15min idle-gap（與 reload 同機制；break 路徑 exit_reason 已非 'walltime' 故跳過、不雙呼叫）。
        if exit_reason == 'walltime':
