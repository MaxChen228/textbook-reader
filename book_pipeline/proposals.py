...
── 全 LLM 閉環：proposals 是 sweep agent 的**決策日誌**（provenance / 稽核軌跡），不再是等人核准的
   佇列。agent 自己分類、寫 normalize 規則/macro/override、過真實數據閘、**self-resolve**（記淨降）。
   操作決策樹是 agent 職責 → 正本在 .claude/skills/book-pipeline/references/math-sweep.md（agent 讀物，
   見記憶 docs-audience-separation：agent 做的事就進 agent references）；本 docstring 只管 proposals
   系統機制（schema/CLI/store）與下方 gate 採用準則。owner 事後稽核 _index.md，可 git revert 任何錯誤
   變更——**真實數據閘 + git 可回退 = 取代人審的安全網**。──

生命週期（agent 自閉環，人退出迴圈）：
  propose   記一筆決策日誌（status=proposed）
  → 分類＋寫修法（規則/macro/override，決策樹見 math-sweep.md）
  → gate    真實數據閘（嚴格淨降且無書上升；edge-case collateral 須在同變更內補 override 掉）
  → resolve 自記決議（accepted=<規則名/commit>＋淨降數；rejected=<理由代碼>）→ self-commit

採用 macro/normalize 規則的強制閘（缺一不可）：
  ...
  (c) `proposals gate` 全 corpus：嚴格淨降且無書上升（collateral 列出 → 補 override 後重跑）
