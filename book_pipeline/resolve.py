...
故交回 agent 判斷**。本模組退成工具：
  ...
  - 落盤：`commit`（resolved / status not_found|version_unavailable|review，flock 並發安全，只收書單 target；
    寫顯式 status 欄。版本判斷另落 editions/<slug>.json，不在此）。
...
def cmd_commit(args) -> int:
    """落盤你的判斷 → crawl_resolution.json（flock 並發安全；寫顯式 status 欄）。模式：
      resolved：--id <id> --hash <hash>（+選填 --title/--author/--mb；寫 status:'resolved'）
      --status not_found：z-lib 真無此書/解答 → **永不再查**（取代舊 --absent；解答本常見）
      --status version_unavailable [--recheck-after <ISO>]：書在但無對應版次 → **可重查**
                （recheck_after 到期自動回工作母體；不降版、不永久放棄——殺「只有別版→永久放棄」僵化）
      --status review（或 --review）：歧義、需架構師裁決（建議同時 proposals propose --domain crawl）
      （--absent 為 --status not_found 的向後相容別名，過渡保留。）
    只接受書單 target slug（拒絕寫非 target，杜絕 ghost）。
    註：版本/解答對齊（edition/sol_alignment）不在此落盤——那是 editions/<slug>.json（見 editions.py），
    由書單管理 skill 多源 LLM 親查後寫入；本 commit 只落「態 + z-lib 連結」。"""
    t = _find_target(args.slug)
    if not t:
        return _emit({'error': f'{args.slug} 非書單 target → 拒絕落盤（不寫 ghost）'}) or 2

    resolved = bool(args.id or args.hash)
    status = getattr(args, 'status', None)
    if getattr(args, 'absent', False):            # legacy 別名 → not_found
        status = status or 'not_found'
    if getattr(args, 'review', False):            # legacy 別名 → review
        status = status or 'review'
    if resolved and status:
        return _emit({'error': '模式衝突：(--id+--hash) 與 --status/--absent/--review 不可並用'}) or 2
    valid = ('not_found', 'version_unavailable', 'review')
    ...
    if resolved:
        ...
        # 下載前書況閘（D4 護欄不可繞）：候選書名鐵定配錯（main 抓週邊 / 書名零重疊）→ 拒落盤，
        # 逼改挑或 --status review。純書名比對、零誤判；治本省後續整條鏈浪費。
        qcres = resolution_qc(t, args.title or '')
        if qcres['block'] and not args.force:
            return _emit({'error': '書況閘擋下：此候選鐵定配錯書 → 改挑候選或 --status review（確認無誤可 --force）',
        ...
        return _emit({'error': '須 --id+--hash（resolved）| --status not_found|version_unavailable|review 之一'}) or 2
...
def main() -> int:
    ...
    p = sub.add_parser('commit', help='落盤決策：--id+--hash | --status not_found|version_unavailable|review')
    p.add_argument('slug')
    p.add_argument('--id')
    p.add_argument('--hash')
    ...
    p.add_argument('--status', choices=('not_found', 'version_unavailable', 'review'),
                   help='非 resolved 落盤：not_found(永久) | version_unavailable(可重查) | review(裁決)')
    p.add_argument('--recheck-after', dest='recheck_after', default=None,
                   help='version_unavailable 的重查到期 ISO 時戳（到期自動回工作母體）')
