...
# 重判斷（audit/catalog_audit/sol_extract）high、視覺 qc low。crawl 解析 2026-06 升級為「多源查證」
# （booklist-manager skill：每本書 fan-out 多 haiku subagent 交叉查證連結+版本）→ 鎖 claude-only——
# subagent fan-out 是 claude code 獨有，codex headless 無法 spawn claude haiku（故不走 codex 家族、
# 不適用 effort）。env BOOK_PIPELINE_PROVIDER_CHAIN 仍可臨時凌駕（但設非 claude → skill 的 fan-out 失效）。
